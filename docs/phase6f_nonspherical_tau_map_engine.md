...
**Status:** Implemented with strict data gates. **Scientific M33 τ-map claims blocked** until primary Corbelli 2014 maps are acquired.
...
| Mode | CLI | Behavior |
|------|-----|----------|
| **Strict (default)** | `build_phase6f_nonspherical_tau_map.py` | Stops with `BLOCKED_PENDING_PRIMARY_CORBELLI_MAPS` if G1/G2 fail |
...
| Gate | Requirement |
|------|-------------|
| G1 | Primary Corbelli VLA+GBT HI under `data/raw/phase6f/primary/corbelli2014_hi/` |
| G2 | Primary BVIgi stellar map under `.../corbelli2014_stellar_mass/` |
| G3 | Tilted-ring geometry CSV |
| G4 | WCS / grid metadata |
| G5 | Documented units |
| G6 | Uncertainty / mask rules |
| G7 | Provenance registry |

**scientific_ready** = G1, G2, G3, G4, G5, G7 all PASS.
...
Phase **4A/4B** remain **axisymmetric radial extensions** (`τ_2D = τ_radial(R)`). This engine is the **non-spherical disk-plane** reconstruction path for Phase 6F-impl once primary data pass gates.
