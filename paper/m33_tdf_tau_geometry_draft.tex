% Phase 6E — submission package polish (journal-neutral; M33 TDF tau-geometry)
...
% Figures: prefer paper/figures/ (Phase 6E), else outputs/figures/, else placeholder.
\newcommand{\includefig}[2][]{%
  \def\figfn{#2}%
  \IfFileExists{figures/\figfn}{%
    \includegraphics[#1]{figures/\figfn}%
  }{%
    \IfFileExists{../outputs/figures/\figfn}{%
      \includegraphics[#1]{../outputs/figures/\figfn}%
    }{%
      \IfFileExists{#2}{%
        \includegraphics[#1]{#2}%
      }{%
...
% TODO (Phase 6E): Replace with verified author names, affiliations, and corresponding author.
\author{%
  \textbf{[Author One]}$^{1}$\thanks{Corresponding author: \texttt{[email@institution.edu]}} \and
  \textbf{[Author Two]}$^{2}$ \and
  \textbf{[Author Three]}$^{1,2}$\\[0.5em]
  {\small
  $^{1}$[Department, Institution, City, Country]\\
  $^{2}$[Department, Institution, City, Country]\\
  \textit{Phase 6E submission package --- author block intentionally incomplete}
  }%
}
\date{\today}
...
\begin{figure}[ht]
  \centering
  \includefig[width=0.88\linewidth]{fig1_rotation_baryonic.png}
...
\begin{figure}[ht]
  \centering
  \includefig[width=0.88\linewidth]{fig2_model_comparison.png}
...
\begin{figure}[ht]
  \centering
  \includefig[width=0.88\linewidth]{fig3_tau_gradient.png}
...
\begin{figure}[ht]
  \centering
  \includefig[width=0.44\linewidth]{fig4_tau_map_disk.png}
  \hfill
  \includefig[width=0.44\linewidth]{fig4_tau_map_sky.png}
...
\begin{figure}[ht]
  \centering
  \includefig[width=0.78\linewidth]{fig5_deflection_proxy.png}
...
Processed data, configuration, and analysis scripts for this study are maintained in the project repository.
The canonical rotation table is \texttt{data/processed/m33\_rotation.csv} (Corbelli et al.\ 2014 Table~1; D1-derived baryonic velocities with PASS\_WITH\_CAVEAT).
Reproducibility commands are listed in \texttt{outputs/reports/phase6a\_reproducibility\_commands.md}.
Phase~6A--6E audits (\texttt{run\_phase6a\_publication\_audit.py} through \texttt{run\_phase6e\_submission\_package\_audit.py}) enforce claim control; no new science results are introduced in packaging phases.

\section*{Acknowledgments}
\label{sec:acknowledgments}

% TODO (Phase 6E): Add funding agencies, compute resources, and colleague acknowledgments.
[Acknowledgments placeholder: funding sources, institutions, and helpful discussions to be added before journal submission.]

\section*{Conflict of Interest}
\label{sec:coi}

The authors declare no competing interests.
% TODO (Phase 6E): Update if any author has relevant disclosures.

\section*{Funding}
\label{sec:funding}

% TODO (Phase 6E): Add grant numbers and funding body names when available.
[Funding placeholder: specify grants and supporting institutions before submission.]
